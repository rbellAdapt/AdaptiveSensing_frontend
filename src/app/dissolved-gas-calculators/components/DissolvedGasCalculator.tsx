...
export default function DissolvedGasCalculator() {
  ...
  const [gasEntries, setGasEntries] = useState<GasEntry[]>([
    { id: "1", name: "O2", value: 20.946, unit: "Percent" },
    { id: "2", name: "Ar", value: 0.934, unit: "Percent" },
    { id: "3", name: "CO2", value: 420, unit: "ppmv" },
    { id: "4", name: "CH4", value: 1000, unit: "ppmv" },
    { id: "5", name: "C2H6", value: 50, unit: "ppmv" },
  ]);
  ...
  const calculateDissolvedGas = async () => {
    setLoading(true);
    setError(null);
    try {
      // 1. Calculate mole fractions for all explicit inputs
      let balanceMoleFraction = 1.0;
      let moleFractions: number[] = [];
      let gasNames: string[] = [];
      ...
      for (const entry of gasEntries) {
        let mf = 0;
        switch (entry.unit) {
          case "Mole Fraction": mf = entry.value; break;
          case "Percent": mf = entry.value / 100; break;
          case "ppthv": mf = entry.value / 1e3; break;
          case "ppmv": mf = entry.value / 1e6; break;
          case "ppbv": mf = entry.value / 1e9; break;
          case "pptv": mf = entry.value / 1e12; break;
        }

        if (duplicateCheck.has(entry.name)) {
            throw new Error(`Duplicate gas "${formatGasLabel(entry.name)}" found. Please rename.`);
        }
        duplicateCheck.add(entry.name);

        moleFractions.push(mf);
        gasNames.push(entry.name);
        balanceMoleFraction -= mf;
      }

      if (balanceMoleFraction < 0) {
          throw new Error("Sum of gases is more than 100%");
      }

      // Add the balance gas
      moleFractions.push(balanceMoleFraction);
      gasNames.push(balanceGas);
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      calculateDissolvedGas();
    }, 500); // 500ms debounce
    return () => clearTimeout(timer);
  }, [temp, tempUnit, salt, saltUnit, pressure, pressureUnit, reportingUnit, gasEntries, balanceGas]);
            ...
            <div className="p-4 space-y-4">
              ...
              <div className="grid grid-cols-3 gap-2 items-center">
                ...
                >
                  ...
                  <option value="IonicStrength(mol_kg_H2O)">Ionic Strength (mol/kg-soln)</option>
                  <option value="IonicStrength(mol_kg_soln)">Ionic Strength (mol/kg-H2O)</option>
              ...
              {/* Balance Gas */}
              <div className="grid grid-cols-3 gap-2 items-center">
                <label className="text-sm text-slate-300 pt-2 text-right opacity-70">Balance Gas</label>
                <div className="col-span-2 mt-2 border-l border-slate-800 pl-2">
                  <select
                    value={balanceGas}
                    onChange={(e) => setBalanceGas(e.target.value)}
                    className="w-full bg-slate-950/50 border border-slate-800 rounded px-2 py-1.5 text-xs text-slate-400 focus:outline-none focus:border-[var(--color-cyan)]"
                  >
                    {GAS_OPTIONS.map((opt) => (
                      <option key={opt.value} value={opt.value}>{opt.label}</option>
                    ))}
              ...
              <div className="space-y-2">
                {gasEntries.map((entry) => (
                  ...
                  >
                    ...
                    <select
                      value={entry.name}
                      onChange={(e) => updateGasEntry(entry.id, "name", e.target.value)}
                    ...
                    <input
                      ...
                      step="any"
                      value={entry.value}
                      onChange={(e) => updateGasEntry(entry.id, "value", parseFloat(e.target.value) || 0)}
                    ...
                    <select
                      value={entry.unit}
                      onChange={(e) => updateGasEntry(entry.id, "unit", e.target.value)}
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="font-mono text-right md:-translate-x-4">
                          <span className="text-white">{prop ? prop.value : "---"}</span>
                  ...
                  {results?.concG?.["nice name"]?.map((name, idx) => (
                    ...
                    >
                      <span className="text-[var(--color-cyan)] font-mono">
                        {formatGasLabel(name)} {idx === results.concG!["nice name"].length -1 ? "(Bal)" : ""}
                      </span>
