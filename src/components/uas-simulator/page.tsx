...
export default function Home() {
    ...
    if (activeTab === 'sensors' && liveSensorConfig) {
      const timeoutId = setTimeout(() => {
        ...
        const enhancedConfig = {
          ...
          enable_proximity_clustering: liveAdaptiveConfig?.enableProximityClustering ?? false
        };
  ...
  return (
    <div className="min-h-screen bg-[#0d1117] text-[#b0c4de] p-2 lg:p-4 font-sans selection:bg-[#00e5ff] selection:text-[#0d1117] bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-[#00e5ff]/5 via-[#0d1117] to-black">
      <header className="mb-2 border-b border-[#00e5ff]/30 pb-2 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-extrabold tracking-tight text-[#b0c4de]">
            UAS <span className="text-[#00e5ff] drop-shadow-[0_0_8px_rgba(0,229,255,0.6)]">Plume Simulator</span>
          </h1>
          <p className="opacity-70 mt-1 text-xs tracking-wide">Adaptive Decision Support for Environmental Monitoring</p>
        </div>
        <div className="flex items-center gap-4 text-xs font-mono opacity-80 backdrop-blur-sm bg-black/20 p-2 rounded-full border border-green-500/30">
